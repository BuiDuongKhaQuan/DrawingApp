...
public class UR {
	...
	public void redo(ArrayList<Shape> cshape) {
		try {
			...
			if (object.getPositions().equals("")) {
				...
				object.setPositions(object.getPositions() + "=" + t.getPositions());
			}
	...
	private void DetectShapeType(int[] oldpos, Item object, ArrayList<Shape> cshapes, Boolean filled) {
		...
		if (object.getName() == "circle") {
			...
			((Triangle) cshapes.get(object.getI())).setxPoints(new int[] { oldpos[2], oldpos[3], oldpos[4] });
			((Triangle) cshapes.get(object.getI())).setyPoints(new int[] { oldpos[5], oldpos[6], oldpos[7] });
